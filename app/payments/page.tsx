...
export default function PaymentsPage() {
  ...
  function ParamsInitializer() {
    ...
    useEffect(() => {
      ...
      // 이미 폼이 열려 있다면 재설정을 방지
      if (!showPaymentForm && certificateId && service === 'certificate_issue') {
        const data = {
          certificateId,
    ...
    // searchParams 객체 대신 queryString(primitive)을 의존성으로 사용해 불필요한 재실행 방지
    }, [isHydrated, isLoggedIn, router, queryString, showPaymentForm])

    return null
  }

  // SSR/CSR 불일치 방지를 위해 클라이언트 마운트 후 한 번만 생성
  useEffect(() => {
    if (!hasGeneratedKeysRef.current && isHydrated && isLoggedIn) {
      const generatedOrderId = `order_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
      const generatedCustomerKey = user?.userId
        ? `customer_${user.userId}`
        : `guest_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
      setOrderId(generatedOrderId)
      setCustomerKey(generatedCustomerKey)
  ...
  // 항상 초기화 컴포넌트를 마운트하여 쿼리 파라미터를 반영
  const Initializer = (
  ...
  // 아직 결제 폼 조건이 충족되지 않은 경우: 초기화만 수행하며 가벼운 플레이스홀더 렌더
  return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      {Initializer}
      <div className="text-sm text-gray-500">결제 준비 중...</div>
    </div>
  )
}
