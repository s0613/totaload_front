...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await userService.login({ usernameOrEmail: email, password });
      // 새 LoginResponse 구조에서 직접 사용자 정보 추출 또는 /auth/me 호출
      if (res.accessToken) {
        // 토큰이 있으면 응답에서 직접 사용자 정보 사용
        login({
          email: res.email,
  ...
  const handleGoogleLogin = () => {
    // 즉시 navigation - state 업데이트로 인한 리렌더링 방지
