...
import CarDamageMarker from "@/components/car-damage-marker";
...
export default function CertificateDetail({ certificateId }: CertificateDetailProps) {
  ...
  // 체크리스트 항목별 사진 (백엔드에서 조회)
  const vehicleImages = certificate?.photos && certificate.photos.length > 0
    ? certificate.photos.map(photo => photo.cloudFrontUrl)
    : ["/placeholder.jpg"]; // 사진이 없으면 placeholder 표시
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg border p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-orange-700">도막 수치 검사 결과</h3>
                  </div>
                  <p className="text-sm text-gray-600 mb-4">
                    아래 다이어그램은 도막 측정기로 측정한 결과, 수치가 높은 부위를 표시합니다.
                    X 표시된 부위는 도장 수리 또는 교체 이력이 있을 수 있습니다.
                  </p>
                  <CarDamageMarker
                    initialMarkedParts={certificate?.damagedParts || []}
                    readOnly={true}
                  />
                </div>
              </div>
