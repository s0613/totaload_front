...
export interface Certificate {
  ...
  issuedByName?: string; // 발급자 성명
  issuedByCompany?: string; // 발급자 소속회사
  issuedAt?: string; // 발급 시각
  issuerUserId?: number; // 발급자 User ID
  imageUrls?: string[]; // 이미지 URL 리스트
  photos?: Photo[]; // 체크리스트 항목별 사진 목록
  damagedParts?: string[]; // 도막 수치가 높은 부위 목록
}
...
export class CertificateService {
  ...
  static async getMyCertificates(): Promise<Certificate[]> {
    try {
      console.log('내가 발급받은 인증서 조회 시작');

      const response = await api.get('/certificates/my-received');
